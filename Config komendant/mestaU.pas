...
begin
  ...
  if MessageDlg('Подтвердите введенные данные'+#13#10+GetArrayValuesAsString1+' ' +GetArrayValuesAsString2,
  ...
  begin
    ...
    // Создаем и выполняем запрос для добавления данных в базу данных
      ...
      try
        ...
        query1.SQL.Text := 'INSERT INTO balki (Номер, "Кол_во_мест") VALUES (:Номер, :КолвоМест)';

        for i := 0 to balki - 1 do
        begin
          query1.ParamByName('Номер').AsInteger := arr[i, 0];
          query1.ParamByName('КолвоМест').AsInteger := arr[i, 1];
          query1.ExecSQL;
        end;
      finally
        query1.Free;
    ...
    //Создаем и выполняем запрос для добавления данных в базу данных
        ...
        try
          ...
          query2.SQL.Text := 'INSERT INTO obchaga (Номер, "Кол_во_мест") VALUES (:Номер, :КолвоМест)';

          for i := 0 to obchaga - 1 do
          begin
            query2.ParamByName('Номер').AsInteger := arr2[i, 0];
            query2.ParamByName('КолвоМест').AsInteger := arr2[i, 1];
            query2.ExecSQL;
          end;
        finally
          query2.Free;
      ...
      begin
        ...
        try
          query2.Connection := connection;
          query2.SQL.Text := 'INSERT INTO obchaga (Номер, "Кол_во_мест", "ФИО_сотрудника") VALUES (:Номер, :КолвоМест, :ФИО)';

          query2.ParamByName('Номер').AsString :='0';
          query2.ParamByName('КолвоМест').AsString :='0';
          query2.ParamByName('ФИО').AsString :='нету';
          query2.ExecSQL;
